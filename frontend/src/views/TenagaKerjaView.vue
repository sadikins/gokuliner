...
const filteredCostItems = computed(() => {
  return allCostItems.value.filter(item => item.kategori === 'Tenaga Kerja');
});
...
<template>
  <div class="">
    <h2 class="text-3xl font-bold mb-6">Manajemen Tenaga Kerja</h2>


    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 bg-white rounded-xl p-6 border border-gray-200 h-full flex flex-col">
        <h3 class="text-xl font-semibold mb-4">Daftar Tenaga Kerja</h3>
        <div class="flex-grow">
          <TenagaKerjaList
            :tenagaKerjas="filteredCostItems"
            @edit="editBahanBaku"
      ...
      <div class="lg:col-span-1 bg-white rounded-xl p-6 border border-gray-200">
        <h2 class="text-2xl font-semibold text-gray-700 mb-5" v-if="!isEditing">Tambah Tenaga Kerja</h2>
        <h2 class="text-2xl font-semibold text-gray-700 mb-5" v-else>Edit Tenaga Kerja</h2>
        <form @submit.prevent="handleSubmit()">
          <div class="mb-4">
            <label for="nama" class="block mb-1 text-sm font-medium text-gray-700">Nama Tugas:</label>
            <InputText  id="nama" v-model="formModel.nama" placeholder="Contoh: Koki / Petugas Packing" required/>
          </div>
          <!-- <div class="mb-4">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <InputText type="text" id="satuan_beli" v-model="formModel.satuan_beli" placeholder="Contoh: Jam" required
                  /> </div>
          </div>
          <div class="mb-4">
          ...
          <div class="mb-4 border border-slate-300  border-dashed  p-3 rounded-md bg-subtle">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="netto_per_beli" class="block mb-1 text-sm font-medium text-gray-700">Lama Pengerjaan:</label>
                    <InputText type="number" id="netto_per_beli" v-model.number="formModel.netto_per_beli" min="0" step="0.0001" required
                           />
                </div>
                <div>
                    <label for="satuan_pemakaian" class="block mb-1 text-sm font-medium text-gray-700">Satuan Waktu:</label>
                    <InputText type="text" id="satuan_pemakaian" v-model="formModel.satuan_pemakaian" placeholder="Contoh: Jam / Menit" required
                           />
                </div>
            </div>
          </div>
