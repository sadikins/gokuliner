    ...
    <table class="w-full text-sm text-left text-gray-500">
      <thead class="text-xs text-gray-700 uppercase bg-gray-50">
        <tr>
          ...
          <th class="px-6 py-3">Pemakaian</th>
          <th class="px-6 py-3">Tanggal Dibuat</th>
          <th class="px-6 py-3">Aksi</th>
        </tr>
      </thead>
      <tbody>
        <tr v-if="operasionals.length === 0">
            <td colspan="9" class="px-6 py-24 font-medium text-center text-gray-400">
            Tidak ada data operasional. Tambahkan data operasional.
          </td>
        </tr>
        <tr v-for="(bb, index) in operasionals" :key="bb.id" class="border-b border-slate-200" >
